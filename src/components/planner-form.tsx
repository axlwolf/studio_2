    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <CardContent className="grid gap-6 md:grid-cols-2">
            <FormField name="title" control={form.control} render={({ field }) => (
              <FormItem className="md:col-span-2">
                ...
                <FormControl><Input placeholder="Ej. El ciclo del agua" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField name="grade" control={form.control} render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>{mockSelectOptions.grades.map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
            )} />
            <FormField name="subject" control={form.control} render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>{mockSelectOptions.subjects.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
            )} />
        ...
        <Card>
            ...
            <CardContent className="grid gap-6 md:grid-cols-2">
                <FormField name="contextDiagnosis" control={form.control} render={({ field }) => (
                    <FormItem className="md:col-span-2">
                        <FormLabel>Diagnóstico del Contexto</FormLabel>
                        <FormControl><Textarea placeholder="Describe el contexto del grupo y la comunidad..." {...field} /></FormControl>
                        <FormMessage />
                    </FormItem>
                )} />
                <FormField name="formativeField" control={form.control} render={({ field }) => (
                    <FormItem>
                        <FormLabel>Campo Formativo</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Selecciona un campo" /></SelectTrigger></FormControl>
                            <SelectContent>{mockSelectOptions.formativeFields.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                        </Select>
                        <FormMessage />
                    </FormItem>
                )} />
                <FormField name="articulatingAxis" control={form.control} render={({ field }) => (
                    <FormItem>
                        <FormLabel>Eje Articulador</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Selecciona un eje" /></SelectTrigger></FormControl>
                            <SelectContent>{mockSelectOptions.articulatingAxes.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                        </Select>
                        <FormMessage />
                    </FormItem>
                )} />
                <FormField name="content" control={form.control} render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl><Textarea placeholder="Contenidos del programa a abordar..." {...field} /></FormControl>
                        <FormMessage />
                    </FormItem>
                )} />
                <FormField name="pda" control={form.control} render={({ field }) => (
                    <FormItem>
                        <FormLabel>Procesos de Desarrollo de Aprendizaje (PDA)</FormLabel>
                        <FormControl><Textarea placeholder="PDA a favorecer..." {...field} /></FormControl>
                        <FormMessage />
                    </FormItem>
                )} />
            </CardContent>
        ...
        <div>
            <div className="flex items-center justify-between mb-4">
                ...
                <Button type="button" onClick={handleGenerateSuggestions} disabled={isGenerating} variant="outline" className="text-accent-foreground border-accent hover:bg-accent/10 hover:text-accent-foreground">
                    {isGenerating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4 text-accent" />}
                    Generar Sugerencias con IA
                </Button>
            </div>

            {isGenerating && <p className="text-center text-muted-foreground py-8">Generando ideas... <Loader2 className="inline-block h-4 w-4 animate-spin" /></p>}
                        ...
                        {aiSuggestions.map((suggestion, index) => (
                            <Card key={index} className="bg-primary/5 border-primary/20">
                                ...
                                <CardContent>
                                    ...
                                    <p className="text-sm"><strong className="font-medium">Materiales:</strong> {suggestion.materials}</p>
                    ...
                    watchedValues.activities.map((activity, index) => (
                        <Card key={index}>
                            ...
                            <CardContent>
                               ...
                               <p className="text-sm"><strong className="font-medium">Materiales:</strong> {activity.materials}</p>
                            </CardContent>
                        </Card>
                    ))
                ) : (
